...
export default function ResultVideo({filename,transcriptionItems}) {
  const videoUrl = "https://caption-x.s3.amazonaws.com/"+filename;
  ...
  const load = async () => {
    ...
    await ffmpeg.writeFile('/tmp/notoSans-Regular.ttf', await fetchFile(notoSansRegular));
    setLoaded(true);
  }
  ...
  const transcode = async () => {
    ...
    await ffmpeg.exec([
      '-i', filename,
      '-preset', 'ultrafast',
      '-vf', `subtitles=subs.srt:fontsdir=/tmp:force_style='Fontname=Noto Sans Regular,FontSize=30,MarginV=70,PrimaryColour=${toFFmpegColor(primaryColor)},OutlineColour=${toFFmpegColor(outlineColor)}'`,
      'output.mp4'
    ]);
